"dg2nm" <-
function(x, y = NA, modproj, mlong=NULL, mlat=NULL)
{
#===============================================================================
...
#===============================================================================
  ...
  miss <- function(x){
	...
	if(is.null(mlong)) mlong <- min(x)
	if(is.null(mlat)) mlat <- min(y)
	if(!miss(modproj)) {
		x <- x - mlong
		y.ctr <- y - mlat
		if(modproj == "mean") {
...
# z     variable of interest (i.e. fish density)
# w     appropriate areas of influence set as weighted factors
# plot  if TRUE, the curve expressing (QñQ(T))/Q as a function of T is 
...
#
#===============================================================================
#MAL, 27 Sept.  - R apparently cannot parse "ñ", so I am commenting out code for SA!
####################
...
#    SA <- sum((QT[1:nb]+QT[2:(nb+1)])*w)/Q
    ...
    # computation of (QñQ(T))/Q as a function of T
#    T <- c(0,cumsum(w))
#    T <- T[nb+1] - T
#    T <- rev(T)
#    QT <- QT[nb+1] - QT
#    QT <- rev(QT)
    ...
    # display
#    if(plot)
#        plot(T, (Q-QT)/Q, main="Curve (QñQ(T))/Q", type="o", pch="+")
...
#MAL 27 Sept - had two comment out these final to lines to avoid getting 
#Error in source("Spatial_indicators_functions.r") : Spatial_indicators_functions.r:910:1: unexpected $end